...
## Causative
# !! For generation only -sis- (which will often be wrong)
der -> stem_end   <sis:>    [der=[+cs,-ps,-autoben]]
## Autobenefactive
# !! For generation only -a-
der -> autoben1      [a:]
...
# !! generation: no -i
sbj_tan -> main   [:]      [tm=pst,-neg];[tm=prs];[tm=sub,-neg]
...
# !! generation: ii, not uu
sbj_an -> end    [ii:]     [tm=contemp,-neg]
...
# all begin with -r; -t also possible for 2s, 2p, and 3sf (but omit for !! generation)
prf -> prf1       [r:]
...
# The L seems not to be required with -s; !! generation: leave out
# !! generation: leave out -i
main2 -> end      [s:]     [cnj=s]
# !! generation: leave out -i
main2 -> end      <Lf:>    [cnj=f]
...
# !! generation: dative: -f, instrumental: -n, ablative: -dhaa
noun -> n_conj     [f:]     [case=dat]
noun -> n_dhaa    <dhaa:>
n_dhaa -> n_conj   [:]      [case=abl]
