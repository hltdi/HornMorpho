...
Create Language, Morphology, and POSMorphology objects for Silt'e.

All functions specific to Silt'e morphology are here (or imported
from geez.py).
"""
...
### Various functions that will be values of attributes of Silt'e Morphology
...
def vb_get_citation(root, fs, simplified=False, guess=False, vc_as=False):
    ...
    fs.update(STV.morphology['v'].citationFS if vc_as else STV.morphology['v'].defaultFS)
    # Refreeze the feature structure
    fs.freeze()
    # Find the first citation form compatible with the updated feature structure
    citation = STV.morphology['v'].gen(root, fs, from_dict=False,
...
def root_postproc(root, geez=False):
    ...
    if geez:
        return root2geez(GEEZ_SERA['sil'][1], root, lang='stv')
    else:
#        # Irregular
#        if root == "al_e":
#            return '<al_e>'
...
def n_postproc(analysis):
    ...
    if analysis[0]:
        if not gram1.get('v'):
            ...
            analysis[0] = sera2geez(GEEZ_SERA['sil'][1], analysis[0], lang='stv')

## Create Language object for Silt'e, including preprocessing, postprocessing,
## and segmentation units (phones).
STV = language.Language("Silt'e", 'Stv',
                        postproc=lambda form: sera2geez(GEEZ_SERA['sil'][1], form, lang='stv'),
                        preproc=lambda form: geez2sera(GEEZ_SERA['sil'][0], form, lang='stv'),
                        seg_units=[["a", "A", "e", "E", "i", "I", "o", "O", "u", "U", "M", "w", "y", "'", "_", "|", "*"],
...
## Create Morphology object and noun, verb, and copula POSMorphology objects for Silt'e,
## including punctuation and ASCII characters that are part of the romanization.
STV.set_morphology(language.Morphology(
                                       pos_morphs=[('v',)],
# ('cop', [], [], []), ('n', [], [], []), ('v', [], [], [])],
...
# Functions that simplifies Silt'e orthography
STV.morphology.simplify = lambda word: simplify(word)
STV.morphology.orthographize = lambda word: orthographize(word)

# Function that performs trivial analysis on forms that don't require romanization
STV.morphology.triv_anal = lambda form: no_convert(form)

## Functions converting between feature structures and simple dicts
STV.morphology['v'].anal_to_dict = lambda root, anal: vb_anal_to_dict(root, anal)
STV.morphology['v'].dict_to_anal = lambda root, anal: vb_dict_to_anal(root, anal)

## Default feature structures for POSMorphology objects
## Used in generation and production of citation form
STV.morphology['v'].defaultFS = \
    language.FeatStruct("[pos=v,tm=prf,as=smp,vc=smp,sb=[-p1,-p2,-plr,-fem],ob=[-expl,-p1,-p2,-plr,-fem,-b,-l,-prp,-frm],cj1=None,cj2=None,pp=None,ax=None,-neg,-rel,-sub,-def,-acc,rl=[-p,-acc,-adv,-comp]]")
STV.morphology['v'].FS_implic = {'rel': ['def', 'sub'], 'cj1': ['sub'], 'pp': ['rel', 'sub'], 'def': ['rel', 'sub'], 'l': ['prp'], 'b': ['prp'], 'ob': [['expl']]}
# defaultFS with voice and aspect unspecified
STV.morphology['v'].citationFS = language.FeatStruct("[pos=v,tm=prf,sb=[-p1,-p2,-plr,-fem],ob=[-expl],cj1=None,cj2=None,pp=None,ax=None,-neg,-rel,-sub,-def,-acc,rl=[-p,-acc,-adv,-comp]]")
#STV.morphology['n'].defaultFS = \
#    language.FeatStruct("[pos=n,-acc,-def,-neg,-fem,as=smp,cnj=None,der=[-ass],-dis,-gen,-plr,poss=[-expl,-p1,-p2,-plr,-fem,-frm],pp=None,v=None,vc=smp,rl=[-p,-gen,-acc]]")
#STV.morphology['n'].FS_implic = {'poss': [['expl']]}
# defaultFS with voice and aspect unspecified
#STV.morphology['n'].citationFS = language.FeatStruct("[-acc,-def,-neg,-fem,cnj=None,der=[-ass],-dis,-gen,-plr,poss=[-expl],pp=None,v=inf]")
#STV.morphology['cop'].defaultFS = language.FeatStruct("[cj2=None,-neg,ob=[-expl],-rel,sb=[-fem,-p1,-p2,-plr,-frm],-sub,tm=prs]")

## Functions that return the citation forms for words
STV.morphology['v'].citation = lambda root, fss, simplified, guess, vc_as: vb_get_citation(root, fss, simplified, guess, vc_as)
#STV.morphology['n'].citation = lambda root, fss, simplified, guess, vc_as: n_get_citation(root, fss, simplified, guess, vc_as)

## Functions that convert analyses to strings
STV.morphology['v'].anal2string = lambda fss: vb_anal2string(fss)
#STV.morphology['n'].anal2string = lambda fss: n_anal2string(fss)
#STV.morphology['cop'].anal2string = lambda fss: cop_anal2string(fss)

## Postprocessing function for nouns (treats roots differently)
# STV.morphology['n'].postproc = lambda analysis: n_postproc(analysis)

def load_anal(pos='v', lex=True, guess=False):
    if lex:
        STV.morphology[pos].load_fst(True, verbose=True)
    if guess:
        STV.morphology[pos].load_fst(True, guess=True, verbose=True)

def load_gen(pos='v', lex=True, guess=False):
    if lex:
        STV.morphology[pos].load_fst(True, generate=True, invert=True, verbose=True)
    if guess:
        STV.morphology[pos].load_fst(True, generate=True, invert=True, guess=True, verbose=True)
